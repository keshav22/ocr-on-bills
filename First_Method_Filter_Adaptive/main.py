...
def image_border(path,path_save) :
    list1 = os.listdir(path+"/")
    for item in list1:
        img = Image.open(path+item)
        #img.show()
        img = ImageOps.expand(img,border=3,fill='black')
        img.save(path_save+item)
...
if __name__ == '__main__':
    ...
    input_dir = "./Preprocessing/output_denoise"
    output_dir = "./output/"
    ...
    for im_name in im_names:
        ...
        print()
        f = open(os.path.join(output_dir,file_name[15:len(file_name)]+"_filter_" + str(i) + ".txt"), 'w')
        f.write(result)
        f.close()
        i += 1
# ------------------------------------------------------------------------------------
